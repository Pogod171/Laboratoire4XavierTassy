...
export default class Response {
  constructor(HttpContext) {
    ...
    this.req = HttpContext.req;
    this.errorContent = "";
    this.url = this.HttpContext.req.url
  }
  ...
  JSON(jsonObj, ETag = "", fromCache = false) {
    ...
    if (jsonObj != null) {
      let content = JSON.stringify(jsonObj);
        if (!fromCache && this.req.method == "GET"){
          let decomposedPath = utilities.decomposePath(this.url); 
          if(decomposedPath.isAPI && decomposedPath.id == undefined)
            CachedRequestsManager.add(this.url, content, ETag);
        }
